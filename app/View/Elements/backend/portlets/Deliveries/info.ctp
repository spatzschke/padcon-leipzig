...
function saveAndReloadConfirmationHeader(id) {
	...
	xhr = $.ajax({
		...
		<div class="offerInfo">
			...
			<?php 
				echo $this->Form->create('Delivery', array('div'=>false, 'data-model' => 'Confirmation'));
			?>
			...
			<?php
				...
				echo $this->Form->input('delivery_number', array('label' => false, 'data-model' => 'Confirmation', 'data-field' => 'confirmation_number', 'autoComplete' => false, 'div' => false, 'class' => 'noValid col-md-8'));	
				...
				echo '<div class="controls col-md-12">';
					...
					if(isset($this->data['Confirmation'])) {		
						$this->request->data['Delivery']['modified'] = $this->Time->format($this->data['Confirmation']['modified'], '%d.%m.%Y');
					}
